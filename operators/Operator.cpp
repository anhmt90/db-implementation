...
//			ofs<< "string(t."+get<0>(p).name+".value) == \""+get<1>(p).value+"\"";
//			ofs<< "strcmp(t."+get<0>(p).name+".value, string(\""+get<1>(p).value+"\").c_str()) == 0";
			ofs<< "t."+get<0>(p).name+" == "+get<0>(p).typeStr+"::castString(string(\""+get<1>(p).value+"\").c_str(), string(\""+get<1>(p).value+"\").length())";
		else
			ofs<< "t."+get<0>(p).name+".value == "+get<1>(p).value;
		if (z+1 < pred_size)
			ofs<<" && ";
	}
	ofs << " ) { \n";
