...
int main(int argc, char* argv[]) {
	...
	for(size_t z = 0; z<times; z++){
		...
		try{
			...
			if(_warehouse_itr == warehouse.pk_index.end()) continue;
			...
			if(_customer_itr == customer.pk_index.end()) continue;
			...
			if(_district_itr == district.pk_index.end()) continue;
			...
			for(int index = 0; index < items; ++index){
				...
				if(_item_itr == item.pk_index.end()) continue;
				...
				if(_stock_itr == stock.pk_index.end()) continue;
...
//	for(auto w : warehouse.data){
//		cout << w.w_id << ", " << w.w_name << "\n";
//	}
