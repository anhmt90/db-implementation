...
void SemanticAnalyser::build(){
	...
	ofstream ofs;
	ofs.open("TranslatedQuery.cpp");
	ofs << "#include \"./hpp/TPCC_Schema.hpp\" \n";
	ofs << "#include \"./operators/Operator.hpp\" \n\n";
	ofs << "using namespace std; \n\n";
	ofs << "extern \"C\" void qExecution(TPCC* tpcc){ \n";
	for(auto& rel : query->relations)
		ofs<< "auto& "+ rel.name +" = tpcc->"+rel.name+"; \n";
	...
	for(auto& pro : query->projections){
		print->required.push_back(Attribute(pro.name, pro.typeStr, pro.tablename));
	}
	print->produce();
