...
/*-----------------------------------------------------------------------------------------------------------------------*/
void TPCC::h_Insert(h_Row& row) {
	history.push_back(row);
}

void TPCC::h_Import(ifstream& itbl) {
	string line;
	if (itbl.is_open()) {
		while (getline(itbl, line)) {
			vector<string> elm = split(line);
...
/*-----------------------------------------------------------------------------------------------------------------------*/
void TPCC::o_Insert(tup_3Int tup, o_Row& row) {
	bool done = order_ix.insert(make_pair(tup, order.size())).second;
	if(done)
		order.push_back(row);
}

inline void TPCC::o_Import(ifstream& itbl) {
	string line;
	if (itbl.is_open()) {
		while (getline(itbl, line)) {
			vector<string> elm = split(line);
...
/*-----------------------------------------------------------------------------------------------------------------------*/
void TPCC::ol_Insert(tup_4Int tup, ol_Row row) {
	if(orderline_ix.insert(make_pair(tup, orderline.size())).second)
		orderline.push_back(row);

}

inline void TPCC::ol_Import(ifstream& itbl) {
	string line;
	if (itbl.is_open()) {
		while (getline(itbl, line)) {
			vector<string> elm = split(line);
			...
			bool done = orderline_ix.insert(make_pair(tup, orderline.size())).second;
			if(done)
				orderline.push_back(row);
		}
...
/*-----------------------------------------------------------------------------------------------------------------------*/
void TPCC::i_Insert(Integer i_id, i_Row& row) {
	if(item_ix.insert(make_pair(i_id, item.size())).second)
		item.push_back(row);
}

void TPCC::i_Import(ifstream& itbl) {
	string line;
	if (itbl.is_open()) {
		while (getline(itbl, line)) {
			vector<string> elm = split(line);
...
/*-----------------------------------------------------------------------------------------------------------------------*/
void TPCC::s_Insert(tup_2Int tup, s_Row& row) {
	if(stock_ix.insert(make_pair(tup, stock.size())).second)
		stock.push_back(row);
}

inline void TPCC::s_Import(ifstream& itbl) {
	string line;
	if (itbl.is_open()) {
		while (getline(itbl, line)) {
			vector<string> elm = split(line);
